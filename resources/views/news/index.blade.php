...
<div class="app-content">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($item->image)
                    <img src="{{ asset('uploads/news/' . $item->image) }}" width="80" height="50" style="object-fit: cover; border-radius: 5px;">
                @else
           ...
           <td>{{ \Carbon\Carbon::parse($item->news_date)->format('d M Y') }}</td>
            <td>
                ...
                <form action="{{ route('news.destroy', $item->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this news?');">
                    @csrf
                    @method('DELETE')
